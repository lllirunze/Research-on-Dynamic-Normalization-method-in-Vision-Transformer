...
class UN2dFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, x, weight, bias, running_var, ema_gz, eps, momentum, buffer_x2, buffer_gz, iters, fp_buffer_size,
                bp_buffer_size, warmup_iters, outlier_filtration, skip_counter, mask_x):
        ctx.eps = eps
        ...
        if current_iter <= fp_buffer_size or current_iter < warmup_iters or skip:
            ...
            var = torch.exp(torch.mean(torch.log(buffer_x2), dim=0)).view(1, C, 1,
                                                                          1)  # numerically-stable version of the geometric mean
    ...
    def backward(ctx, grad_output):
        ...
        return gx, (grad_output * y).sum(dim=3).sum(dim=2).sum(dim=0), grad_output.sum(dim=3).sum(dim=2).sum(
            dim=0), None, None, None, None, None, None, None, None, None, None, None, None, None
...
class UN1d(nn.Module):
    ...
    def __init__(self, channels, window_size=4, warmup_iters=4000, outlier_filtration=False, eps=1e-5, momentum=0.9):
        ...
        assert (isinstance(window_size, int)
                ...
                or isinstance(window_size,
                              tuple)), "window_size should be in the type of int/list/tuple, but got {} {}".format(
    ...
    def extra_repr(self):
        return '{num_features}, fp_win={fp_buffer_size}, bp_win={bp_buffer_size}, eps={eps}, momentum={momentum}, warmup={warmup_iters}, outlier_filtration={outlier_filtration}'.format(
            **self.__dict__)
    ...
    def forward(self, x, pad_mask=None):
        ...
        # 判断维度是否是 B x N x C, 若是 N x C 则维度扩张为 1 x N x C
        ...
        if self.training:
            ...
            x = UN2dFunction.apply(x, self.weight, self.bias, self.running_var.view(1, self.num_features, 1, 1),
                                   self.ema_gz.view(1, self.num_features, 1, 1), self.eps,
                                   self.momentum, self.buffer_x2, self.buffer_gz, self.iters,
                                   self.fp_buffer_size, self.bp_buffer_size, self.warmup_iters, self.outlier_filtration,
                                   self.skip_counter, mask_input)
        else:
            B, C, H, W = x.size()
